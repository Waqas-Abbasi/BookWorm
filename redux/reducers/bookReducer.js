...
const bookReducer = (state = defaultList, action) => {
    switch (action.type) {
        ...
        case actionTypes.ADD_SESSION:
            const bookIndex = state.findIndex(item => item.key === action.payload.bookKey);
            state[bookIndex] = {
                ...state[bookIndex],
                sessionList: [...state[bookIndex].sessionList, action.payload],
                pagesRead: action.payload.endingPage,
            };
